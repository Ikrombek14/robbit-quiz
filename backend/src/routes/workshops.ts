...
// Sarlavha boshidagi tartib prefiksini tozalaydi ("1. ", "2) ", "3-workshop. ")
...
async function compactAll(): Promise<void> {
  const ws = await prisma.workshop.findMany({ orderBy: { order: "asc" }, select: { id: true, order: true } });
  const fixes = ws.map((w, i) => ({ id: w.id, from: w.order, to: i })).filter((x) => x.from !== x.to);
  if (fixes.length > 0) {
    await prisma.$transaction(fixes.map((x) => prisma.workshop.update({ where: { id: x.id }, data: { order: x.to } })));
  }
}
...
workshopsRouter.get("/", requirePanel, async (_req, res) => {
  ...
  res.json({
    workshops: workshops.map((w) => ({ ...w, quiz: w.quizId ? (quizMap.get(w.quizId) ?? null) : null })),
  });
});

// Yaratish — faqat admin. So'ralgan o'ringa KIRITILADI, ro'yxat 0..n-1 qayta raqamlanadi.
workshopsRouter.post("/", requireAdmin, async (req, res) => {
  ...
  const others = await prisma.workshop.findMany({
  ...
  await prisma.$transaction(seq.map((sid, i) => prisma.workshop.update({ where: { id: sid }, data: { order: i } })));

  const fresh = await prisma.workshop.findUnique({ where: { id: created.id } });
  res.json({ workshop: fresh ?? created });
});

// Ommaviy: nomlar ro'yxatidan workshoplar qo'shish — faqat admin
const bulkTitlesSchema = z.object({ titles: z.array(z.string()).min(1) });
workshopsRouter.post("/bulk-titles", requireAdmin, async (req, res) => {
  ...
  const agg = await prisma.workshop.aggregate({ _max: { order: true } });
  let nextOrder = agg._max.order != null ? agg._max.order + 1 : 0;
  await prisma.$transaction(
    titles.map((title) => prisma.workshop.create({ data: { title, order: nextOrder++ } })),
  );
  res.json({ created: titles.length });
});

// Ommaviy: PAPKADAN workshoplar yaratish — faqat admin (har quizdan bitta workshop)
const fromFolderSchema = z.object({ folderId: z.string().min(1), stripPrefix: z.boolean().default(true) });
workshopsRouter.post("/from-folder", requireAdmin, async (req: AuthedRequest, res) => {
  ...
  const existing = await prisma.workshop.findMany({ select: { order: true, quizId: true } });
...
workshopsRouter.patch("/reorder", requireAdmin, async (req, res) => {
  ...
  try {
    await prisma.$transaction(
      parsed.data.ids.map((id, i) => prisma.workshop.update({ where: { id }, data: { order: i } })),
    );
    res.json({ ok: true });
  } catch {
...
workshopsRouter.put("/:id", requireAdmin, async (req, res) => {
  const parsed = workshopSchema.safeParse(req.body);
  if (!parsed.success) {
    res.status(400).json({ error: "Ma'lumotlar noto'g'ri" });
    return;
  ...
  if (!existing) {
    res.status(404).json({ error: "Workshop topilmadi" });
    return;
  }
  const { order: requestedOrder, ...rest } = parsed.data;
  const others = await prisma.workshop.findMany({
  ...
  await prisma.$transaction([
    ...
    ...seq.map((sid, i) => prisma.workshop.update({ where: { id: sid }, data: { order: i } })),
  ]);
...
workshopsRouter.patch("/:id/quiz", requireCanCreate, async (req, res) => {
...
workshopsRouter.post("/bulk-delete", requireAdmin, async (req, res) => {
  const ids = Array.isArray(req.body?.ids) ? (req.body.ids as unknown[]).map(String) : [];
  if (ids.length === 0) {
    res.status(400).json({ error: "ids bo'sh" });
    return;
  }
  const r = await prisma.workshop.deleteMany({ where: { id: { in: ids } } });
  await compactAll();
  res.json({ deleted: r.count });
});

// O'chirish — faqat admin
workshopsRouter.delete("/:id", requireAdmin, async (req, res) => {
  try {
